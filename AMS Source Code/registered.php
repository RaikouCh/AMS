...
// Function to generate a 6-digit security code
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $security_code = generateSecurityCode(); // Generate security code
    $hashed_password = password_hash($security_code, PASSWORD_DEFAULT); // Hash security code

    // Validate required fields
    if (empty($first_name) || empty($last_name) || empty($email) || empty($username) || empty($gender) || 
        empty($location) || empty($phone) || empty($dob)) {
    ...
    if (!ctype_digit($phone) || strlen($phone) > 12) {
        echo json_encode(["success" => false, "message" => "Invalid phone number. Only up to 12 digits allowed."]);
        exit();
    }
    ...
    $stmt = $conn->prepare("SELECT id FROM form WHERE username=? OR email=?");
    ...
    $stmt = $conn->prepare("INSERT INTO form (first_name, last_name, email, username, gender, location, phone, dob, password, role, security_code) 
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sssssssssss", $first_name, $last_name, $email, $username, $gender, $location, $phone, $dob, $hashed_password, $role, $security_code);

    if ($stmt->execute()) {
        $stmt->close();
        $conn->close();

        echo json_encode(["success" => true, "message" => "Admin registered successfully!", "security_code" => $security_code]);
        exit();
    } else {
        echo json_encode(["success" => false, "message" => "Registration failed. Database error: " . $stmt->error]);
        exit();
    }
}
?>
...
<div class="container">
    ...
    <form id="signupForm" action="registered.php" method="POST">
        <div class="row">
            <div class="col-md-6">
    ...
    <div class="login-container">
        ...
        <a href="relogin.php" class="login-btn">Login</a>
    </div>
</div>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener("submit", async (event) => {
        ...
        if (!/^\d{1,12}$/.test(phone)) {
            showMessage("❌ Phone number must be numeric and up to 12 digits.", "error");
            return;
        }
        ...
        try {
            ...
            if (data.success) {
                alert(`✅ Admin registered successfully! Your security code is: ${data.security_code}`);
                localStorage.setItem("adminSecurityCode", data.security_code);
                const firstName = encodeURIComponent(formData.get("first_name"));
                const lastName = encodeURIComponent(formData.get("last_name"));
                window.location.href = `confirm.php?firstname=${firstName}&lastname=${lastName}`;
            } else {
