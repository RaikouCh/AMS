...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['id'], $_POST['status'])) {
        ...
        $query = "UPDATE leasing SET status = '$status' WHERE id = '$id'";
        echo $conn->query($query) ? "Reservation status updated successfully." : "Error updating status: " . $conn->error;
    } elseif (isset($_POST['id'])) {
        $id = $conn->real_escape_string($_POST['id']);
        $query = "UPDATE leasing SET status = 'Cancelled' WHERE id = '$id'";
        echo $conn->query($query) ? "Reservation cancelled successfully." : "Error cancelling reservation: " . $conn->error;
    } else {
...
$query = "SELECT id, CONCAT(firstName, ' ', lastName) AS name, email, phone, contactMethod, leasingOption, property, stayPeriodStart, stayPeriodEnd, status, idPicture, message, source FROM leasing ORDER BY id DESC";
...
<div class="container">
                        ...
                        echo "<tr>
                                ...
                                <td>
                                    <div class='action-buttons'>
                                        ...
                                        <button class='btn btn-primary view1-btn' data-id='{$row['id']}'>View</button>
                                    </div>
                                </td>
                              </tr>";
                        $count++;
    ...
    <div class="modal" id="editReserve" tabindex="-1" aria-labelledby="editReserveLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="modal-title" id="editReserveLabel">Reservation Details</h2>
                    <button type="button" class="btn-close close-modal" aria-label="Close"></button>
                </div>
                <div class="modal-body"></div>
                <img id="res-image" src="image/room1.jpg" alt="Reservation Image">
                <p id="res-message">Thank you for your reservation. We will contact you soon.</p>
                <p id="res-source">Source: Facebook</p>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.view1-btn').forEach(button => {
        button.addEventListener('click', function() {
            const image = this.getAttribute('data-image') || 'image/room1.jpg';
            const message = this.getAttribute('data-message') || 'Thank you for your reservation. We will contact you soon.';
            ...
            document.getElementById('res-source').textContent = `Source: ${source}`;
            new bootstrap.Modal(document.getElementById('editReserve')).show();
        });
    });
});
...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll(".delete-btn").forEach(button => {
        button.addEventListener("click", function () {
            const id = this.getAttribute("data-id");
            console.log("Attempting to delete ID:", id); // Debugging output
